...
if __name__ == '__main__':    
    ...
    #psi2 = psi_sqr (psi)
    psi2 = get_init_statesqr(N, x1, x2, maxdim=maxdim)
    #psi2 = npmps.svd_compress_MPS (psi2, maxdim=sys.maxsize, cutoff=0.)
    H_psi = qtt.MPS_to_MPO (psi2)
    H_psi[0] *= g
    H = npmps.sum_2MPO (H0, H_psi)
#------------------------------------------------------------------
    psi2_fit = fit_psi_sqr(psi, psi2, maxdim = maxdim, cutoff = 0.0)
    npmps.to_canonical_form (psi2_fit, oc=8)
 
    H_psi_fit = qtt.MPS_to_MPO (psi2_fit)
    H_psi_fit[0] *= g
    H_fit = npmps.sum_2MPO (H0, H_psi_fit)

    print('fit psi2 MPO dim:',npmps.MPO_dims(H_psi_fit))
    print('psi2 MPO dim:',npmps.MPO_dims(H_psi))

    #H = make_H_GP (H0, psi, psi2, g, maxdim=maxdim)
    #print('Initial psi dim:',npmps.MPS_dims(psi))
    #psi = qtt.normalize_MPS_by_integral (psi, x1, x2, Dim=2)
    pltut.plot_2D_proj (psi2, x1, x2, ax=None, func=absVal, label='psi2_direct')
    pltut.plot_2D_proj (psi2_fit, x1, x2, ax=None, func=absVal, label='psi2_fit')
    print(len(psi),len(psi2))
    print("load fortran psi2 E = ",np.abs(npmps.inner_MPO (psi, psi,  H_psi)))
    print("load fortran fitpsi2 E = ",np.abs(npmps.inner_MPO (psi, psi,  H_psi_fit)))
    print("load fortran total H MPO E = ",np.abs(npmps.inner_MPO (psi, psi, H)))
    print("load fortran  toatal H fit MPO E = ",np.abs(npmps.inner_MPO (psi, psi, H_fit)))
    ...
    H_data = make_H_GP (H0, psi_data, psi2_data, g, maxdim=maxdim)
    H_data_psi = qtt.MPS_to_MPO (psi2_data)
    psi2_data_fit = fit_psi_sqr(psi_data, psi2_data, maxdim = maxdim, cutoff = cutoff)
    H_data_psi_fit = qtt.MPS_to_MPO (psi2_data_fit)
    #print('Initial psi dim:',npmps.MPS_dims(psi))
    #psi = qtt.normalize_MPS_by_integral (psi, x1, x2, Dim=2)
    ...
    print("data inner HMPOE = ",np.abs(npmps.inner_MPO (psi_data, psi_data, H_data_psi)))
    print("data inner H_fitMPOE = ",np.abs(npmps.inner_MPO (psi_data, psi_data, H_data_psi_fit)))
