...
def random_MPS(N, phydim, seed, vdim=1, dtype=np.complex128):
    ...
    for i in range(N):
        if i == 0:
            mps.append (np.random.rand (1, phydim, vdim).astype(dtype))
        elif i == N-1:
            mps.append (np.random.rand (vdim, phydim, 1).astype(dtype))
        else:
            mps.append (np.random.rand (vdim, phydim, vdim).astype(dtype))
    return mps
